...
int main() {
	...
	BatchNorm2D batchNormModule(2); // Create an instance of the BatchNorm2D module
	batchNormModule.forward(input); // Perform the forward pass
	// Print the output tensor values
	Tensor output = batchNormModule.forward(input);
