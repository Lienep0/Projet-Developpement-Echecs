...
class TrainEncoding:
    ...
    def update_policy_head(self,move, board):
    ...
    def write_encode(self, h5_file,PgnFile,index):
        while True:
            ...
            for move in game.mainline_moves():
                value_head = self.update_value_head(game,board)
                entry = self.update_entry(board)
                policy_head = self.update_policy_head(move, board)
                board.push(move)
                buffer+=1
                ...
                if buffer >= 1000:
                    index=self.save_position(h5_file, index,entries,policy_heads,value_heads,buffer)
                    buffer = 0
                    entries = []
                    policy_heads = []
                    value_heads = []
    ...
    def save_position(self, h5_file, index, entries,policy_heads,value_heads,buffer):
        ...
        for file in files:
            if file.endswith(".pgn"): 
                ...
                with open(pgn, 'r') as f :
                    index=self.write_encode(h5_file,f,index)
                    if index>k*10000:
                        print(index)
                        k+=1
                    if index>limit:
...
if __name__ == "__main__":
    ...
    with h5py.File(h5_path, 'a') as f:
        train_encoder.iterate(f,directory_path, limit)